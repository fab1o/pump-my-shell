...
# ========================================================================
typeset -g Z_CURRENT_PROJECT_FOLDER=""
typeset -g Z_CURRENT_PROJECT_SHORT_NAME=""
typeset -g Z_CURRENT_PROJECT_REPO=""
typeset -g Z_CURRENT_PROJECT_SINGLE_MODE=""
typeset -g Z_CURRENT_PACKAGE_MANAGER=""
typeset -g Z_CURRENT_CODE_EDITOR=""
typeset -g Z_CURRENT_CLONE=""
typeset -g Z_CURRENT_SETUP=""
typeset -g Z_CURRENT_RUN=""
typeset -g Z_CURRENT_RUN_STAGE=""
typeset -g Z_CURRENT_RUN_PROD=""
typeset -g Z_CURRENT_PRO=""
typeset -g Z_CURRENT_TEST=""
typeset -g Z_CURRENT_COV=""
typeset -g Z_CURRENT_TEST_WATCH=""
typeset -g Z_CURRENT_E2E=""
typeset -g Z_CURRENT_E2EUI=""
typeset -g Z_CURRENT_PR_TEMPLATE=""
typeset -g Z_CURRENT_PR_REPLACE=""
typeset -g Z_CURRENT_PR_APPEND=""
typeset -g Z_CURRENT_PR_RUN_TEST=""
typeset -g Z_CURRENT_GHA_INTERVAL=""
typeset -g Z_CURRENT_COMMIT_ADD=""
typeset -g Z_CURRENT_GHA_WORKFLOW=""
typeset -g Z_CURRENT_PUSH_ON_REFIX=""
typeset -g Z_CURRENT_DEFAULT_BRANCH=""
typeset -g Z_CURRENT_PRINT_README=""
...
typeset -g dark_gray_cor="\e[38;5;236m"
...
function help() {
  ...
  print ""
  print "  1. to set a project, type:${blue_cor} pro ${reset_cor}"
  print "  2. to clone project, type:${blue_cor} clone ${reset_cor}"
  print "  3. to setup project, type:${blue_cor} setup${reset_cor}"
  print "  4. to run a project, type:${blue_cor} run${reset_cor}"
...
function check_prj_repo_() {
  eval "$(parse_flags_ "check_prj_repo_" "s" "$@")"
  (( check_prj_repo_is_d )) && set -x
  ...
  if command -v gum &>/dev/null; then
    ...
    gum spin --timeout=8s --title "checking repository uri..." -- git ls-remote "${repo}" 2>/dev/tty
    RET=$?
  else
  ...
  if (( $? != 0 )); then
    Z_PROJECT_REPO[$i]=""
    error_msg="repository uri is invalid or no access rights: $repo"
  fi

  print_debug_ "check_prj_repo_ index: $i - repo: $repo - is_s: $get_prj_repo_is_s - error_msg: $error_msg"
...
function check_prj_folder_() {
  eval "$(parse_flags_ "check_prj_folder_" "s" "$@")"
  (( check_prj_folder_is_d )) && set -x

  local i="$1"
  local folder="${2:-$Z_PROJECT_FOLDER[$i]}"

  print_debug_ "check_prj_folder_ index: $i - folder: $folder - is_s: $get_prj_folder_is_s"
  ...
  if [[ -n "$folder" ]]; then
    ...
    print_debug_ "check_prj_folder_: $folder"
    return 0;
  fi
...
function get_prj_folder_() {
  ...
  (( get_prj_folder_is_d )) && set -x

  local i="$1"
  local folder="${2:-$Z_PROJECT_FOLDER[$i]}"

  if (( get_prj_folder_is_s )); then
    check_prj_folder_ -s $i "$folder"
  else
    check_prj_folder_ $i "$folder"
  fi

  if (( $? == 0 )); then
    echo "$Z_PROJECT_FOLDER[$i]"
    return 0;
  fi
...
function check_prj_pkg_manager_() {
  ...
  (( check_prj_pkg_manager_is_d )) && set -x
  ...
  if [[ -z "$pkg_manager" ]]; then
    ...
    local valid_pkg_managers=(
      "npm" "yarn" "pnpm" "bun" "poe" "conda" "cargo"
    )
...
function save_proj_name_() {
  ...
  if [[ -n "$typed_name" ]]; then
    ...
    if (( $? == 0 )); then
      ...
      print "  project name: $typed_name" >&1
...
function save_prj_mode_() {
  ...
  (( save_prj_mode_is_d )) && set -x

  local i="$1"
  local single_mode="$2"
  local name="$Z_PROJECT_SHORT_NAME_TEMP"
  ...
  if (( single_mode )); then
    print "  project mode: single" >&1
  else
    print "  project mode: multiple" >&1
  fi
...
function save_proj_repo_() {
  ...
  if [[ -n "$typed_repo" ]]; then
    ...
    if (( $? == 0 )); then
      ...
      print "  project repository: $typed_repo" >&1
...
function save_pkg_manager_() {
  ...
  if [[ -n "$pkg_manager" ]]; then
    ...
    if (( $? == 0 )); then
      if (( i > 0 )); then
        update_config_ $i "Z_PACKAGE_MANAGER" "$pkg_manager"
        Z_PACKAGE_MANAGER[$i]="$pkg_manager"
      else
        Z_CURRENT_PACKAGE_MANAGER="$pkg_manager"
      fi
      print "  package manager: $pkg_manager" >&1
      return 0;
    fi
  fi
...
function save_prj_() {
  eval "$(parse_flags_ "save_prj_" "afe" "$@")"
  (( save_prj_is_d )) && set -x
  ...
  print "  try running: ${yellow_cor}${Z_PROJECT_SHORT_NAME[$i]}${reset_cor}" >&1
...
function open_prj_for_pkg_() {
  ...
  if [[ -n "$folder" && -d "$folder" ]]; then
    if [[ -f "$folder/package.json" || -d "$folder/.git" || -f "$folder/pyproject.toml" ]]; then
      return 0;
    fi
...
function check_pkg_silent_() {
  ...
  if [[ -n "$folder" && -d "$folder" ]]; then
    if [[ -f "$folder/package.json" || -d "$folder/.git" || -f "$folder/pyproject.toml" ]]; then # || -f "$folder/environment.yml" || -f "$folder/Cargo.toml"  ]]; then
      return 0;
    fi
    ...
    while [[ "$folder" != "/" ]]; do
      ...
      folder="$(dirname "$folder")"
      print_debug_ "check_pkg_silent_ folder: $folder"
    done
  fi
...
function setup() {
  ...
  print_debug_ " proj_arg=$proj_arg - proj_folder=$proj_folder - folder_arg=$folder_arg"
...
function rev() {
  ...
  if [[ -d "$full_rev_folder" ]]; then
    ...
    if command -v gum &>/dev/null; then
      gum spin --title "cloning... $proj_repo" -- git clone $proj_repo "$full_rev_folder" 2>/dev/tty
    else
...
function get_clone_default_branch_() { # $1 = repo uri # $2 = folder # $3 = branch to clone
  ...
  if command -v gum &>/dev/null; then
    ...
    gum spin --title "determining the default branch..." -- git clone "$1" "$2/.temp"
  else
  ...
  print_debug_ "get_clone_default_branch_ default_branch: $default_branch - my_branch: $my_branch"
  ...
  print_debug_ "get_clone_default_branch_ after fact default_branch: $default_branch - my_branch: $my_branch"
  ...
  print_debug_ "get_clone_default_branch_ default_branch_choice: $default_branch_choice"
...
function clone() {
  ...
  for i in {1..9}; do
    if [[ "$proj_arg" == "${Z_PROJECT_SHORT_NAME[$i]}" ]]; then
      found=$i
      proj_repo="$(get_prj_repo_ -s $i)"
      print_debug_ "clone proj_repo: $proj_repo"
      if [[ -z "$proj_repo" ]]; then return 1; fi
  ...
  print_debug_ "clone single_mode: $single_mode - proj_arg: $proj_arg - branch_arg: $branch_arg - proj_repo: $proj_repo - proj_folder: $proj_folder"
  ...
  if (( single_mode )); then
    ...
    if command -v gum &>/dev/null; then
      gum spin --title "cloning... $proj_repo on $branch_arg" -- git clone $proj_repo "$proj_folder" 2>/dev/tty
      echo "   cloning... $proj_repo on $branch_arg"
    else
  ...
  if [[ -z "$branch_arg" ]]; then
    if [[ -z "$(ls -A "$proj_folder" 2>/dev/null)" ]]; then # first time user is cloning
  ...
  print_debug_ "clone single_mode: branch_arg: $branch_arg"
  ...
  print_debug_ "clone single_mode: default_branch: $default_branch"

  local branch_to_clone_folder="${branch_arg//\\/-}"
  branch_to_clone_folder="${branch_to_clone_folder//\//-}"

  if command -v gum &>/dev/null; then
    gum spin --title "cloning... $proj_repo on $branch_arg" -- git clone $proj_repo "$proj_folder/$branch_to_clone_folder" 2>/dev/tty
    echo "   cloning... $proj_repo on $branch_arg"
  else
    print "  cloning... $proj_repo on $branch_arg"
...
function fetch() {
  ...
  for config in $(git config --get-regexp "^branch\." | awk '{print $1}'); do
    ...
    if ! echo "$current_branches" | grep -q "^$branch_name$"; then
      print_debug_ "fetch cleaning branch_name: $branch_name"
...
function select_branch_() {
  ...
  print_debug_ "select_branch_ : git branch $filter | grep -i $3"
...
function select_pr_() {
  ...
  print_debug_ "select_pr_ : count=$count"
...
function co() {
  ...
  if (( co_is_b || co_is_c )); then
    print_debug_ "co -b or -c asking for: $@"
    local branch="$1"
    local base_branch="$2"
  ...
  git checkout -b "$branch" "$base_branch" # create branch
  if (( $? != 0 )); then return 1; fi
...
function prune() {
  ...
  for config in $(git config --get-regexp "^branch\." | awk '{print $1}'); do
    ...
    if ! echo "$current_branches" | grep -q "^$branch_name$"; then
      print_debug_ "prune cleaning branch_name: $branch_name"
      git config --remove-section "branch."$branch_name"" &>/dev/null
    fi
  done
...
function load_config_entry_() {
  ...
  for key in "${keys[@]}"; do
    ...
    if [[ -z "$value" ]]; then
      case "$key" in
        # Z_PROJECT_SINGLE_MODE) # on clone we want to let the user select the mode if nothing is set on the config
        #   value="0"
        #   ;;
        Z_PACKAGE_MANAGER)
          value="npm"
